### O backend está todo em um docker(configurado com dockerfile), tanto o banco de dados quanto a api em si.

#### Para rodar localmente basta dar o comando:

`docker compose up --build`

assim ele buildará o banco de dados e a api em um docker com a seguinte url:

`http://localhost:5000/`

O backend foi construído com Express js, Prisma ORM e Typescript.

- Ele possui Relacionamento de anel com portador. E há também a model de histórico para ver o histórico de portadores de um anel. Porém essas rotas não estão implementadas.
- Ele possui testes com jest das duas services implementadas no backend.
- Ele possui autenticação com JWT e criptografia da senha do usuário.
- Ele possui todas as rotas de funcionalidades como get dos anéis, criação de anéis, update de anéis e exclusão de anéis.
- Possui validação de criação de anel de acordo com quem é Forjado.
- Possui as ações de criar conta e Login.
- Todas as rotas relacionadas aos anéis são protegidas pelo login(as rotas requisitam o token gerado ao logar)
- Faço o upload de imagem e salvamento na pasta upload dentro da propria api (Foi feito assim para que não houvesse necessidade de conexão com terceiros,dificultando testes da api na avaliação do teste)

O Front end Foi contruido com Next js 14, ShadCn Ui, Tailwind,Typescript, react hook form e zod

- Ele apresenta tela de criação de conta e login, validando formulários e utilizando server actions para realizar
- Os formulários são controlas e validados com react-hook-form e zod
- Nele foi implementada todas as funções de Criar conta, Login, criar um anel, listagem de anéis, update das informações de um anel e exclusão de um anel.
- implementado upload de imagem
- implementado animações de modais, tab, sheets (para abertura de hamburguer menu)
- todas as telas com responsivdade
<details>
<summary>Regras Do desafio</summary>
